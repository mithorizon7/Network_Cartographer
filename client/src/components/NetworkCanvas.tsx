...
export function NetworkCanvas({ scenario, activeLayer, selectedDeviceId, onDeviceSelect }: NetworkCanvasProps) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-md bg-gradient-to-br from-background to-muted/30" data-testid="network-canvas">
      ...
      >
        ...
        {routerPos && scenario.devices.filter(d => d.type !== "router").map(device => {
          ...
          return (
            <line
              ...
              x2={pos.x}
              y2={pos.y}
              className={`stroke-[1] transition-all duration-200 ${
                isSelected 
        ...
        {activeLayer === "network" && routerPos && (
          <g>
            ...
            <foreignObject x={routerPos.x + 10} y={80} width={120} height={40}>
              <div className="flex items-center gap-1.5 rounded bg-chart-5/10 px-2 py-1 text-xs font-medium text-chart-5">
        ...
        {activeLayer === "transport" && routerPos && (
          <g className="pointer-events-none">
            {[
              { label: "HTTPS", offset: -30, color: "text-green-500" },
      ...
      <div className="absolute bottom-4 left-4 rounded-md bg-card/80 px-3 py-2 text-xs backdrop-blur-sm" data-testid="scenario-info">
